...
async function convert(output, tree, extname, processor, name) {
    return new Promise((resolve, reject) => {
        ...
        output.formats.forEach((format) => {
            ...
            fs.ensureDirSync(core.path.dirname(outPath));
            processor.addToTree({
                tree,
                outputOptions: {
        ...
        if (core.SavableAssetCache.has(tree.path)) {
            const cache = core.SavableAssetCache.get(tree.path);
            cache.transformData.files[0].paths.push(...paths);
            core.SavableAssetCache.set(tree.path, cache);
        }
        else {
...
function ffmpeg(options) {
    const defaultOptions = core.merge(true, {
        inputs: [],
        outputs: [],
    }, options);
    return {
        ...
        test(tree, _p, optionOverrides) {
            const opts = core.merge(true, defaultOptions, optionOverrides);
            if (!opts.inputs.length) {
                throw new Error('[ffmpeg] No inputs defined');
            }
            return core.checkExt(tree.path, ...opts.inputs);
        },
        async transform(tree, processor, optionOverrides) {
            // merge options with defaults
            const opts = core.merge(true, defaultOptions, optionOverrides);
            const extname = core.path.extname(tree.path);
            const promises = [];
...
function audio(options) {
    ...
    defaultOptions = core.merge(true, defaultOptions, options);
